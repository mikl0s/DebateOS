...
| D13a | **Forum storage: a thin `store` repository interface + sqlc-generated queries.** Default backend **SQLite** (pure-Go `modernc.org/sqlite`, libSQL-compatible) for dev and v1.0 production; **Postgres** is an optional drop-in backend behind the same interface for future scale. No heavyweight ORM. Search via **SQLite FTS5** in v1.0, abstracted for a later Postgres `tsvector` impl. | Single static binary + one DB file = leanest possible deploy; swappable without over-engineering; in-memory SQLite for tests. |
