...
| D19 | **Development methodology: TDD with very high coverage.** Tests are written before implementation across all code (resolver, CLI, Forum, translators where testable, UI logic). The resolver targets near-total coverage — every resolution rule, conflict scenario, and edge case (including the Phase 0 variant-study corpus) exists as a test before the behavior is implemented. Deterministic builds and WASM/native parity are verified by automated tests, not inspection. | Owner directive (2026-06-12). The resolver's correctness IS the product; explainable resolution demands a test per explanation. The edge-case corpus from Omarchy/CachyOS/Garuda research feeds the test harness directly. |
| D20 | **Phase 0 research scope includes an Arch-variant substitution study (CachyOS, Garuda Linux)** alongside the Omarchy deep-dive — validating same-base foundation swaps, informing the Arch translator's variant-profile/multi-repo design, and harvesting real-world resolver edge cases. See `08`. | Owner directive (2026-06-12). Gives D9's "structure for 1–2 Arch variants" concrete targets and evidence; expands the translation layer without bloat (declarative variant profiles, not forks). |
