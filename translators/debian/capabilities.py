"""
capabilities.py — Debian translator capability gate (DEB-01 / DEB-03 / SC-3).
...
Differs from Arch translator:
- Points to translators/debian/capabilities.json (not arch's)
- Error message says "Debian translator" (not "Arch translator")
- Does NOT declare mkinitcpio/limine/pacman-AUR tokens (DEB-03 Finding 3/4):
  opinions requiring those tokens correctly CapabilityError on Debian.
...
class CapabilityError(Exception):
    ...
    that the Debian translator has not implemented.
...
def check_capabilities(resolved: dict, opinions_index: dict, capabilities: set) -> list:
    """Enforce the capability gate (DEB-01 / SC-3).
    ...
    for opinion_id in resolved.get("applied", []):
        ...
        if status == "required":
            ...
            raise CapabilityError(
                ...
                f"which is not declared by the Debian translator "
                f"at composition time. "
                f"Add it to translators/debian/capabilities.json when implemented. "
