"""
manifest.py — BuildManifest dataclass for DebateOS translators (foundation-neutral).

Aggregates the full payload from a ResolvedSpeech + opinion bodies into a
single typed in-memory structure that the profile emitter and the installer
script consume.

Key design decisions:
- install_order from ResolvedSpeech is authoritative; never re-sort.
- check_capabilities is NOT called here — it is foundation-specific and must
  be called by the CALLER (generator.py) BEFORE BuildManifest.from_resolved.
  This makes the module foundation-neutral: each translator's generator.py
  runs its own capability gate before delegating to this shared manifest builder.
- custom_repos with sig_level="Never" or "OptionalTrustAll" are flagged in
  trust_warnings (T-02-02 / T-04-01).
- SOURCE_DATE_EPOCH is derived deterministically from resolved-speech bytes
  (BLD-03 groundwork) via derive_source_date_epoch.

Source: Adapted from translators/arch/manifest.py — removed the
``from capabilities import check_capabilities`` import and the in-method
``check_capabilities(...)`` call, making the module foundation-neutral.
The Arch generator.py now calls check_capabilities() explicitly before
BuildManifest.from_resolved().
"""
...
class BuildManifest:
    ...
    Foundation-neutral: this class does NOT run the capability gate. The
    CALLER (each translator's generator.py) must call check_capabilities()
    before invoking BuildManifest.from_resolved().
    ...
    ------
    foundation : str
        Target foundation from the resolved speech (e.g. "arch", "debian").
    ...
    trust_warnings : List[str]
        ...
        or "OptionalTrustAll" (T-02-02 / T-04-01).
    ...
    def from_resolved(
        ...
        IMPORTANT: The capability gate (check_capabilities) is NOT run here.
        The CALLER must run check_capabilities(resolved, opinions_index, capabilities)
        BEFORE calling this method. This design makes the manifest module
        foundation-neutral (each translator has its own capabilities module).

        Steps:
        1. Iterate over install_order aggregating all payload fields.
        2. Derive source_date_epoch from resolved_bytes.
        ...
        Args:
            ...
            capabilities: Set of declared capability token strings.
                Accepted here for signature compatibility but the gate is
                caller-responsibility (see above).
        ...
        for opinion_id in install_order:
            ...
            if exec_phase == "first-run":
                ...
                # systemd oneshot unit generation.
            ...
            # Custom repos + trust warnings for sig_level=Never/OptionalTrustAll (T-02-02 / T-04-01)
